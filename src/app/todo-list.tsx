...
function TodoRow({
  ...
  return (
    <li
      className={`flex items-start gap-4 p-4 ${isFirst ? "" : "border-t-2 border-hairline"}`}
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-[10px] uppercase tracking-[0.16em]">
          {todo.due_date && (
        ...
        {breakdownError && (
          <p className="mt-2 border-l-2 border-hot bg-hot/10 px-2 py-1 text-[10px] uppercase tracking-[0.14em] text-foreground">
            {breakdownError}
          </p>
        )}
      </div>
...
function DueDateBadge({
  ...
  const isOverdue = !completed && due < todayStr;
  const isToday = !completed && due === todayStr;
